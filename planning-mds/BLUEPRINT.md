...
- `ArtifactIndexEntry` — one entry per evidence artifact, keyed by an ID of `{run_id}/{artifact_kind}/{12 hex of path digest}`. Identity derives from location within the run, so re-indexing is idempotent; `content_hash` is a separate attribute used for duplicate linking and staleness, deliberately not for identity (ADR-006).
