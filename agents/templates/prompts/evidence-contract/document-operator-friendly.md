...
Resuming an in-flight run in a new session: do NOT generate a new `DOC_RUN_ID` and do NOT re-create the run. Run `python3 agents/scripts/resume-brief.py --run-id <DOC_RUN_ID>` first — it reports position, next gate, recorded decisions, current story, and scope in one read, so the session does not re-derive them. `init-run.py --resume` reuses the existing run folder.
